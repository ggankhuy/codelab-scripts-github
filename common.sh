...
function prompt_t2_with_ip () {
	...
	if [[ $1 == $GAME_DOOM ]] ; then
		echo "./yeti_streamer -policy_config_file lan_policy.proto_ascii -connect_to_game_on_start -direct_webrtc --console_stderr -external_ip="$IP_TO_DISPLAY
                ...
                echo "./dev/bin/yeti_streamer --policy_config_file dev/bin/lan_policy.proto_ascii \
		-connect_to_game_on_start  -direct_webrtc_ws -external_ip=127.0.0.1 -port 44750 &> \
