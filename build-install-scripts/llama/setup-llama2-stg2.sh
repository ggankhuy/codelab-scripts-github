...
if [[ $SOFT_LINK == 1 ]] ; then
    for i in  libmkl_intel_lp64 libmkl_gnu_thread libmkl_core; do
        ln -s \
        $CONDA_PREFIX/pkgs/mkl-2023.1.0-h213fc3f_46344/lib/$i.so.2 \
        $CONDA_PREFIX/pkgs/mkl-2023.1.0-h213fc3f_46344/lib/$i.so.1
    done
else
    for i in  libmkl_intel_lp64 libmkl_gnu_thread libmkl_core; do
        rm -rf $CONDA_PREFIX/pkgs/mkl-2023.1.0-h213fc3f_46344/lib/$i.so.1
        cp \
        $CONDA_PREFIX/pkgs/mkl-2023.1.0-h213fc3f_46344/lib/$i.so.2 \
        $CONDA_PREFIX/pkgs/mkl-2023.1.0-h213fc3f_46344/lib/$i.so.1
    done
